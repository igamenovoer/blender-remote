...
try:
    ...
    def test_full_workflow():
        ...
        print("\n2. Creating objects...")
        cube_name = scene_manager.add_cube(location=(0, 0, 0), size=2.0, name="WorkflowCube")
        sphere_name = scene_manager.add_sphere(location=(3, 0, 0), radius=1.0, name="WorkflowSphere")
        cylinder_name = scene_manager.add_cylinder(location=(-3, 0, 0), radius=0.8, depth=2.0, name="WorkflowCylinder")
    ...
    def test_data_type_functionality():
        ...
        camera = blender_remote.CameraSettings(
            location=[5, -5, 3],
            target=[0, 0, 0]
        )
        ...
        render = blender_remote.RenderSettings(
    ...
    def test_convenience_functions():
        ...
        print(f"Created scene manager: {scene_manager.client.host}:{scene_manager.client.port}")
        
        # Test create_asset_manager with auto-client
        asset_manager = blender_remote.create_asset_manager(port=6688)
        print(f"Created asset manager: {asset_manager.client.host}:{asset_manager.client.port}")
        
        # Test with existing client
        scene_manager2 = blender_remote.create_scene_manager(client)
        print(f"Created scene manager with existing client: {scene_manager2.client.host}:{scene_manager2.client.port}")
    ...
    def run_all_tests():
        ...
        if passed == total:
            print("🎉 All integration tests passed! Python Control API is working correctly.")
        else:
