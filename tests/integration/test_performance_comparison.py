...
class PerformanceTester:
    ...
    def compare_performance(self, command_description, command_func, iterations=5, tolerance_factor=2.0):
        """Compare performance between both services."""
        print(f"⏱️ Performance test: {command_description}")
        
        # Measure BLD_Remote_MCP
        bld_stats, bld_error = self.measure_execution_time(self.bld_remote_client, command_func, iterations)
        
        # Measure BlenderAutoMCP
        auto_stats, auto_error = self.measure_execution_time(self.blender_auto_client, command_func, iterations)
        ...
        if bld_stats and auto_stats:
            ...
            print(f"  BLD_Remote_MCP: {bld_stats['mean']:.4f}s (±{bld_stats['stdev']:.4f}s)")
            print(f"  BlenderAutoMCP: {auto_stats['mean']:.4f}s (±{auto_stats['stdev']:.4f}s)")
            print(f"  Performance ratio: {ratio:.2f}x ({'✅ PASS' if ratio <= tolerance_factor else '❌ FAIL'})")
        else:
            result['performance_ratio'] = None
            result['performance_acceptable'] = False
            print(f"  Performance comparison failed - BLD error: {bld_error}, Auto error: {auto_error}")
...
def performance_tester(dual_services):
    ...
    tester = PerformanceTester(
        dual_services['bld_remote_port'],
        dual_services['blender_auto_port']
    )
...
"""
        ...
        assert result['performance_ratio'] is None or result['performance_ratio'] <= 3.0, \
    ...
    def test_concurrent_operation_handling(self, performance_tester):
        ...
        def rapid_sequential_commands(client):
            ...
            return client.execute_python("result = sum([var_0, var_1, var_2, var_3, var_4])")
...
def test_performance_summary(dual_services):
    ...
    try:
        test_cases = [
            ...
            ("Import statement", lambda c: c.execute_python("import math; result = math.pi")),
            ("Blender API call", lambda c: c.execute_python("import bpy; result = bpy.app.version_string")),
        ...
        for result in results:
            if result['performance_ratio'] is not None:
                ...
                print(f"{result['command']:20} {result['performance_ratio']:6.2f}x {status}")
