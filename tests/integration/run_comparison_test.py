...
def compare_services(test_name, code):
    ...
    # Test BLD_Remote_MCP
    bld_success, bld_result = test_service_with_client(
        6688, "BLD_Remote_MCP", test_name, code
    )

    # Test BlenderAutoMCP
    auto_success, auto_result = test_service_with_client(
        9876, "BlenderAutoMCP", test_name, code
    )
...
def main():
    ...
    test_cases = [
        ...
        (
            "Blender API Call",
            "import bpy; bpy.ops.mesh.primitive_cube_add(); result = 'cube_added'",
        ),
    ]
