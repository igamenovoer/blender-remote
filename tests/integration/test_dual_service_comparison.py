...
class DualServiceTester:
    ...
    def setup_environment(self):
        ...
        if not self._is_port_available(self.bld_remote_port):
            raise RuntimeError(f"Port {self.bld_remote_port} (BLD_Remote_MCP) is not available")
        if not self._is_port_available(self.blender_auto_port):
            raise RuntimeError(f"Port {self.blender_auto_port} (BlenderAutoMCP) is not available")
        
        print(f"✅ Ports available - BLD Remote: {self.bld_remote_port}, BlenderAuto: {self.blender_auto_port}")
    ...
    def _verify_service_startup(self):
        ...
        # Test BLD_Remote_MCP
        bld_remote_ok = self._test_port_connection(self.bld_remote_port, "BLD_Remote_MCP")
        
        # Test BlenderAutoMCP  
        blender_auto_ok = self._test_port_connection(self.blender_auto_port, "BlenderAutoMCP")
        
        if not bld_remote_ok:
            raise RuntimeError(f"BLD_Remote_MCP service not responding on port {self.bld_remote_port}")
        if not blender_auto_ok:
            raise RuntimeError(f"BlenderAutoMCP service not responding on port {self.blender_auto_port}")
    ...
    def _test_port_connection(self, port, service_name):
        ...
        try:
            ...
            if result == 0:
                ...
                print(f"❌ {service_name} port {port}: TCP connection failed (code: {result})")
                return False
        except Exception as e:
            print(f"❌ {service_name} port {port}: Exception during connection test: {e}")
            return False
...
class TestDualServiceComparison:
    ...
    def test_basic_connectivity(self, dual_service_setup):
        ...
        assert tester.blender_auto_client is not None, "BlenderAutoMCP client not created"
    ...
    def test_simple_python_execution(self, dual_service_setup):
        ...
        results = tester.execute_command_on_both("Simple Python execution", execute_simple_code)
        ...
        assert 'error' not in str(bld_result).lower(), f"BLD_Remote_MCP returned error: {bld_result}"
        assert 'error' not in str(auto_result).lower(), f"BlenderAutoMCP returned error: {auto_result}"
    ...
    def test_blender_api_access(self, dual_service_setup):
        ...
        results = tester.execute_command_on_both("Blender API access", access_blender_api)
...
"""
    ...
    def test_multiple_sequential_commands(self, dual_service_setup):
        ...
        commands = [
            ...
            ("Variable access", lambda c: c.execute_python("result = test_var + ' world'")),
        ...
        for cmd_name, cmd_func in commands:
            ...
            assert results['both_succeeded'], f"Command '{cmd_name}' failed on one or both services: {results}"
...
"""
        ...
        results = tester.execute_command_on_both("Large code execution", execute_large_code)
