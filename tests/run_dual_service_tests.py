...
def main():
    ...
    parser.add_argument("--unit", action="store_true",
                       help="Run unit tests only (single service)")
    ...
    if args.unit:
        test_suites.append(("Unit Tests", ["python", "-m", "pytest", "tests/test_bld_remote_mcp.py"]))
    elif args.integration:
        test_suites.append(("Integration Tests", ["python", "-m", "pytest", "tests/integration/"]))
    elif args.performance:
        test_suites.append(("Performance Tests", ["python", "-m", "pytest", "tests/integration/test_performance_comparison.py", "-m", "slow"]))
    elif args.quick:
        test_suites.extend([
            ("Unit Tests", ["python", "-m", "pytest", "tests/test_bld_remote_mcp.py"]),
            ("Basic Integration", ["python", "-m", "pytest", "tests/integration/test_dual_service_comparison.py", "-m", "not slow"])
        ])
    else:
        # Run all tests
        test_suites.extend([
            ("Unit Tests", ["python", "-m", "pytest", "tests/test_bld_remote_mcp.py"]),
            ("Integration Tests", ["python", "-m", "pytest", "tests/integration/test_dual_service_comparison.py"]),
            ("Performance Tests", ["python", "-m", "pytest", "tests/integration/test_performance_comparison.py"])
        ])
    ...
    if failed == 0:
        print("\n🎉 ALL TESTS PASSED!")
        print("✅ BLD_Remote_MCP appears to be functionally equivalent to BlenderAutoMCP")
        exit_code = 0
    else:
