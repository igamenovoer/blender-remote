...
class BlenderMCPClient:
    ...
    def __init__(self, host: Optional[str] = None, port: int = 6688, timeout: float = 30.0):
        """
    ...
    def _receive_full_response(self, sock: socket.socket, buffer_size: int = 8192) -> bytes:
        ...
        try:
            ...
            if not data:
                raise BlenderConnectionError("Connection closed before receiving any data")
    ...
    def execute_command(self, command_type: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                raise BlenderTimeoutError(f"Connection timeout after {self.timeout} seconds")
            except socket.error as e:
                raise BlenderConnectionError(f"Failed to connect to {self.host}:{self.port}: {str(e)}")
            ...
            try:
                ...
                raise BlenderTimeoutError(f"Receive timeout after {self.timeout} seconds")
            ...
            raise BlenderMCPError(f"Unexpected error during command execution: {str(e)}")
        finally:
            if sock:
                try:
                    sock.close()
