...
def test_protocol_format(port, service_name, message_format, test_name):
    ...
    try:
        ...
        is_success = (
            response.get('response') == 'OK' or 
            response.get('status') == 'success' or
            'error' not in response.get('status', '').lower()
        )
...
def main():
    ...
    bld_with_bld_format = test_protocol_format(6688, "BLD_Remote_MCP", bld_remote_format, "BLD_Remote format")
    bld_with_auto_format = test_protocol_format(6688, "BLD_Remote_MCP", blender_auto_format, "BlenderAuto format")
    ...
    auto_with_bld_format = test_protocol_format(9876, "BlenderAutoMCP", bld_remote_format, "BLD_Remote format")
    auto_with_auto_format = test_protocol_format(9876, "BlenderAutoMCP", blender_auto_format, "BlenderAuto format")
    ...
    print(f"  {format_result(bld_with_auto_format[0], 'BlenderAuto format (compatibility)')}")
    
    print("\nBlenderAutoMCP Protocol Support:")
    print(f"  {format_result(auto_with_bld_format[0], 'BLD_Remote format (compatibility)')}")
    ...
    if not bld_with_auto_format[0]:
        print("   🔧 Add BlenderAuto protocol support to BLD_Remote_MCP for full compatibility")
